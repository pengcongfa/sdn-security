...
public class SwitchResourceBase extends ServerResource {
    ...
    protected List<OFStatistics> getSwitchStatistics(String appId,String appKey,long switchId,
        ...
        if(appId.equals("1")){
            ...
            if(accessCheck.appAuthentication(appId, appKey)){
                if(accessCheck.appPermissionSetFinder(appId, PermissionType.read_statistics)){
                	 if(xacmlCtr.getXacmlEvaluateResult(appId)){
                		 if (sw != null) {
                          ...
                          log.info("---------应用ID："+appId+"-------权限检查失败：无 read_statistics 权限");
                 	 }
               }
              else{
    ...
    protected List<OFStatistics> getSwitchStatistics(String appId,String appKey,String switchId, OFStatisticsType statType) {
        return getSwitchStatistics(appId,appKey,HexString.toLong(switchId), statType);
    }
