...
public class SwitchStatisticsResource extends SwitchResourceBase {
    ...
    public Map<String, Object> retrieve() {
        ...
        if (statType.equals("port")) {
            values = getSwitchStatistics(appId,appkey,switchId, OFStatisticsType.PORT);
        } else if (statType.equals("queue")) {
            values = getSwitchStatistics(appId,appkey,switchId, OFStatisticsType.QUEUE);
        } else if (statType.equals("flow")) {
            values = getSwitchStatistics(appId,appkey,switchId, OFStatisticsType.FLOW);
        } else if (statType.equals("aggregate")) {
            values = getSwitchStatistics(appId,appkey,switchId, OFStatisticsType.AGGREGATE);
        } else if (statType.equals("desc")) {
            values = getSwitchStatistics(appId,appkey,switchId, OFStatisticsType.DESC);
        } else if (statType.equals("table")) {
            values = getSwitchStatistics(appId,appkey,switchId, OFStatisticsType.TABLE);
