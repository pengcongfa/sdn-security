...
IFlowReconcileListener, IInfoProvider {
    ...
    @Override
    public Iterator<? extends IDevice> queryDevices(String appId,String appKey,Long macAddress,
                                                    Short vlan,
                                                    Integer ipv4Address,
                                                    Long switchDPID,
                                                    Integer switchPort) {
    	if(appId.equals("1")){
            ...
            if(accessCheck.appAuthentication(appId, appKey)){
                if(accessCheck.appPermissionSetFinder(appId, PermissionType.read_topology)){
                	 ...
                	 logger.info("---------应用ID："+appId+"-------权限检查失败：无 read_topology 权限");
                 	 }
               }
              else{
    ...
    public void init(FloodlightModuleContext fmc) throws FloodlightModuleException {
        ...
        this.accessCheck=fmc.getServiceImpl(IAppInfoStorageService.class);
        this.xacmlCtr=fmc.getServiceImpl(IXacmlCtrService.class);
