...
public abstract class AbstractStorageSource 
    ...
    protected IAppInfoStorageService accessCheck;
    protected IXacmlCtrService xacmlCtr;
    ...
    @Override
    public Future<?> insertRowAsync(String appId,String appKey,final String tableName,
    	   ...
    	   if(appId.equals("1")){
    		   Future<?> future = executorService.submit(
    	        ...
    	        if(accessCheck.appAuthentication(appId, appKey)){
    	            if(accessCheck.appPermissionSetFinder(appId, PermissionType.flow_mod_route) ||
    	            		accessCheck.appPermissionSetFinder(appId, PermissionType.set_flow_priority)){
    	            	 if(xacmlCtr.getXacmlEvaluateResult(appId)){
    	            		  ...
    	            		  logger.info("---------应用ID："+appId+"-------未通过基于属性的XACML访问控制");
    	                      }
    	              }
    	             else{
    	            	 logger.info("---------应用ID："+appId+"-------权限检查失败：无 flow_mod_route/ set_flow_priority权限");
    	             	 }
    	           }
    	          else{
    	        	  logger.info("---------应用ID："+appId+"-------身份认证失败");
    	              }
    	        return null;
    	    }
    }
    ...
    @Override
    public Future<?> deleteRowAsync(String appId,String appKey,final String tableName, final Object rowKey) {
    	 ...
    	 if(appId.equals("1")){
    	        ...
    	        if(accessCheck.appAuthentication(appId, appKey)){
    	            if(accessCheck.appPermissionSetFinder(appId, PermissionType.flow_mod_drop)){
    	            	 if(xacmlCtr.getXacmlEvaluateResult(appId)){
    	            	        Future<?> future = executorService.submit(
    	            	            new StorageRunnable() {
    	            	                public void doStorageOperation() {
    	            	 ...
    	            	 logger.info("---------应用ID："+appId+"-------权限检查失败：无 flow_mod_drop 权限");
    	             	 }
    	           }
    	          else{
    	        	  logger.info("---------应用ID："+appId+"-------身份认证失败");
    ...
    public void init(FloodlightModuleContext context)
        ...
        accessCheck=
           context.getServiceImpl(IAppInfoStorageService.class);
        xacmlCtr=
        	context.getServiceImpl(IXacmlCtrService.class);
        counterStore =
